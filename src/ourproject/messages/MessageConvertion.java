...
public class MessageConvertion {
	...
	public static InterMessage bytesToMessage(byte[] bytes){
		...
		if(myBuffer.get(0)==UPDATE_POLY_MSG){
			...
			int verCnt = (bytes.length-1)/4;
			
			//Log.e("","verCnt: "+verCnt);
			
			for(int i=0;i<1000;i++){
				
				short arg1 = myBuffer.getShort(i*4+1);
				short arg2 = myBuffer.getShort(i*4+3);
				...
				if(i==0)
					...
					if(arg1==myBuffer.getShort(1) && arg2==myBuffer.getShort(3))
						break;
				}
				
			}
			
			message = new BoundsUpdateMsg(poly);
		}
	...
	public static byte[] messageToBytes(BoundsUpdateMsg Message){
		...
		ByteBuffer myBuffer = ByteBuffer.allocateDirect(poly.getSize()*4+1);
		myBuffer.put(UPDATE_POLY_MSG);
		
		//Log.e("","messageToBytes: "+poly.getSize());
		
		for(int i=0;i<poly.getSize();i++){
			myBuffer.putShort(1+i*4, poly.getPoint(i).getx());
			myBuffer.putShort(3+i*4, poly.getPoint(i).gety());
		}
		
		byte[] res = new byte[poly.getSize()*4+1];
